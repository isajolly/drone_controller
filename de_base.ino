...
if (difference_init<10000){
  ...
  pulse_length_escA = REDUCE_MOTOR_A*( COEF_THROTTLE*throttle_pulse_length - 0*COEF_PITCH_ROLL*roll_pulse_length - 0*COEF_PITCH_ROLL*pitch_pulse_length + 0*COEF_YAW*yaw_pulse_length );

  pulse_length_escB = REDUCE_MOTOR_B*( COEF_THROTTLE*throttle_pulse_length + 0*COEF_PITCH_ROLL*roll_pulse_length - 0*COEF_PITCH_ROLL*pitch_pulse_length - 0*COEF_YAW*yaw_pulse_length );

  pulse_length_escC = REDUCE_MOTOR_C*( COEF_THROTTLE*throttle_pulse_length - 0*COEF_PITCH_ROLL*roll_pulse_length + 0*COEF_PITCH_ROLL*pitch_pulse_length - 0*COEF_YAW*yaw_pulse_length );

  pulse_length_escD = REDUCE_MOTOR_D*( COEF_THROTTLE*throttle_pulse_length + 0*COEF_PITCH_ROLL*roll_pulse_length + 0*COEF_PITCH_ROLL*pitch_pulse_length + 0*COEF_YAW*yaw_pulse_length );
